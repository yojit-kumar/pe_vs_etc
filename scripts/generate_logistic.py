...
Usage
-----
    python scripts/generate_logistic.py [options]

    -h, --help            show this help message and exit
    --a-min   FLOAT       left edge of parameter sweep  (default: 3.5)
    --a-max   FLOAT       right edge of parameter sweep (default: 4.0)
    --n-param INT         number of a values            (default: 500)
    --L       INT [...]   one or more series lengths    (default: 100000 1000000)
    ...
    --noise   FLOAT [...] measurement noise σ values    (default: 0.0 0.01 0.05 0.1)
    --workers INT         parallel worker processes     (default: all CPU cores)
    --outdir  PATH        output directory              (default: data/)
    --seed    INT         RNG seed                      (default: 42)

Example — quick test run
------------------------
    python scripts/generate_logistic.py --n-param 50 --L 10000 --workers 4
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Logistic map — PE vs ETC parameter sweep",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument('--a-min',   type=float, default=3.5,
    ...
    p.add_argument('--workers', type=int,
                   default=None,   # None → mp.cpu_count()
...
def setup_logging(outdir: str) -> None:
    ...
    log_path = Path(outdir) / f"logistic_{time.strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    ...
    logging.info(f"Logistic map sweep: a ∈ [{args.a_min}, {args.a_max}], "
    ...
    for L in args.L:
        filename = str(outdir / f"logistic_L{L}.h5")
        run_experiment(
            map_name       = 'logistic',
            filename       = filename,
            param_values   = a_values,
            fixed_params   = {},          # logistic has no fixed params
            L              = L,
